...
def fetch_flathub_stats():
    ...
    for name in ["CoreseCommand", "CoreseGui"]:
        resp_flathub = rq.get(
            f"https://klausenbusk.github.io/flathub-stats/data/fr.inria.corese.{name}.json"
        )
        ...
        data.append(
            {
                "today": today,
                "installs_total": None,
                "installs_last_month": sum(
                    [_arches_downloads(e) for e in downloads[-30:]]
                ),
                "installs_last_7_days": sum(
                    [_arches_downloads(e) for e in downloads[-7:]]
                ),
                "name": name,
            }
        )
